...
- `src/lib/render.ts` / `src/lib/detail.ts` - **isomorphic HTML renderers** for event/location/news cards and detail blocks. Card rules worth keeping: **the sign-up count is never shown to a player** - the chip read "12/18 ekipa" and the detail page "6 slobodnih mjesta", and a venue reported it backfiring (2026-09-08: "idem se prijavit i vidim da imaju samo 3 ekipe... i neću se ni prijavit"), so `eventStatus` and `freeSpots` now speak capacity only, and only name the number once it is small: `Popunjeno` at zero, `Još 4 mjesta` down to one, `5+ slobodnih mjesta` above the `SPOTS_NAMED_BELOW = 5` threshold in format.ts, and `Prijave otvorene` when the quiz has no cap at all (there the count is the only figure there is, so nothing is said). A quiet quiz and a busy one are deliberately indistinguishable above the threshold; below it the number is genuine urgency and worth printing. The figures are still in the API and in the public `data/events.json` - this is an interface rule, not a secret.  an event with no name is titled by its **venue** (most termini are unnamed, so a generic fallback produced three identical "Pub kviz" cards in a row); the date block is the only place the date appears, and the eyebrow returns only for `danas`/`sutra`; chips sit on a rule at the card foot with the body centred, so equal-height rows read as padding rather than a hole. Used by Astro at build time and by `src/scripts/live.ts` in the browser, which re-renders `[data-live]` lists from `/data/*.json` on every page load (so the cron-refreshed snapshots show without a rebuild) and upgrades dates to "danas/sutra". The detail pages additionally redraw their **About block** (`[data-about]`, on both `/lokacije/{id}-{slug}/` and `/dogadaji/{id}/`), which is why that block is always rendered and merely `hidden` when empty: it was static HTML, so a venue description edited in the admin waited for the next deploy and one venue sat on the site two revisions behind. Removal is asymmetric and `aboutUpdate()` in `detail.ts` owns the rule - a snapshot reports no description both when the venue has none and when the cron lost that venue's detail request, so a description is only *removed* when `meta.json` says `locationDetails: "complete"`.
- **THE INVARIANT, and the bug class it exists to stop: a page must agree with the snapshot it is
  shown, not with the snapshot it was built from.** This site renders the same data twice, in Astro
  at deploy time and in the browser from the cron snapshot, so *anything* a template derives from
  event data is frozen at deploy unless something revisits it. The list pages got this right from
  the start (they recompute their counters on the `ukp:live` event); the venue detail page never
  subscribed, and on 2026-09-08 Pub 022 and Yesterday advertised their own cards under the heading
  "Trenutno nema zakazanih termina", offered a CTA leading away from the quiz on screen, and shared
  a `?prijava` link that did nothing, because all three were decided at build time. The same day,
  the 404 page's event fallback was found to be a header and a panel only, so a quiz scheduled
  after the deploy lost its fact tiles, the venue blurb and the whole actions row (Kako doći,
  WhatsApp, Podijeli). **When you add anything to a detail page that depends on events.json, put it
  in a shared renderer in `lib/detail.ts` and have `live.ts` re-apply it** - `locationEventsView()`
  (heading + CTA + `?prijava` target, applied by `applyLocationEvents`) and `eventActionsHTML()` /
  `sectionHeadHTML()` are the examples. `tests/e2e/location-detail.spec.ts` and `event-detail.spec.ts`
  hold the invariant directly: they serve a snapshot that deliberately disagrees with the build and
  assert the page follows the snapshot, in both directions, plus that an unreachable snapshot
  changes nothing. A failed refresh must never blank a page - only ever add or correct.
...
- **Location URLs and slugs.** A venue page lives at `/lokacije/{id}-{slug}/` under the slug the build's `slugify()` (format.ts) made. The cron's PHP `slugify()` must produce the identical string, since every card the browser redraws links with it: iconv transliteration once spelled the "•" in "VIVA Caffe • Lounge • Bar" as the letter o, and the venue was a 404 from every list. The build also emits a slug-free alias `/lokacije/{id}/` (`src/pages/lokacije/[id].astro`, meta refresh + `location.replace`, noindex). **A venue created in the admin after the last deploy has no page at all** - `getStaticPaths` only builds what was in the snapshot on disk - and until 2026-09-08 the 404 page only hopped such a link to that alias, which is missing too, so OUT Rooftop (159) and Royal Palace (160) were unreachable from the moment they were created, `?prijava` included. The 404 page now looks the id up in `locations.json` and either forwards to the address the snapshot names (a venue renamed since the deploy) or, when that already is the address, renders the whole page from the snapshot (`locationDetailHTML` in detail.ts, which mirrors `[slug].astro` and shares its `nearbyLocations` rule and the `icon()` set exported from render.ts); `?prijava` forwards to whichever quiz is next before anything is drawn. With no snapshot to go on it still falls back to the by-id hop. Same idea as the event/news fallbacks that were already there - keep `locationDetailHTML` and `[slug].astro` in step.
...
- `scripts/fetch-data.mjs` (build) and `server/cron/refresh-data.php` (host cron) produce the **same JSON shape**; keep them in sync when adding fields (the last one added was `feeCurrency` / `defaultFeeCurrency`, EUR or BAM: the site printed "25 € po ekipi" for a quiz in Grude priced in marks until both fetchers carried it and `fee()` learned to write KM). **Location detail is one request per venue and both fetchers used to lose some of them silently**, which cost eleven venues their description, fee and team cap on the live site for a day: the cron cached a partial map for 24 h and the build ran six requests in flight with no pause, and ~140 of those against a 600/min per-IP limit earns scattered 429s. Both now pace themselves (cron 4/s serial, build 3 in flight plus a serial retry sweep), retry, and report the outcome as `meta.json`'s `locationDetails: "complete" | "partial"` - which the browser reads before it is willing to remove anything. The cron additionally only stamps its cache complete when every venue resolved, so a bad run is retried on the next tick instead of owning the whole day. **The detail cache rolls rather than sweeping**: the map used to be re-read once every 24 h, so a venue description edited in the admin sat a day behind a snapshot that was otherwise fifteen minutes old (and so did the fee, the caps and the deadline). Each run now re-reads every venue it has never seen plus the twelve oldest (`UKP_DETAIL_SLICE`, whole map round in ~11 runs), a venue whose request failed is never stamped read so it stays first in line, and the selection lives in `server/cron/refresh-lib.php` with `ukp_description()` - tested through the real PHP by `tests/unit/cron-refresh.test.ts`. `description` itself no longer waits for any of that: it rides `GET /api/pub-quiz-locations` as of 2026-09-06, so every run has it; the cached detail is only the fallback for an older API, and once the list is seen to carry descriptions it is authoritative about their absence too, which is how a blurb deleted in the admin leaves the site. Wherever the list and the detail carry the same field (address, start time, WhatsApp link) the list wins, since it is the one read every run. Both drop events more than 3 h past their start (the API's `from=<today>` still returns this morning's quiz, so a 03:00 termin was being advertised as "danas") and recompute each location's next quiz from what is genuinely ahead. `getLocations()` then sorts locations with a scheduled quiz first: a visitor came to find a quiz, not to read an alphabetical directory.
...
- `deploy.sh` mirrors the build **with deletion** (`--delete`), leaving `data/` to the cron (neither uploaded nor deleted), so a page built for an event that has since gone does not outlive the next deploy. `--dry-run` lists what would be removed; read it before a real deploy after long gaps. **`img/api/` is excluded from that mirror only to protect it from `--delete`** - the cron writes `<id>.jpg` there for artwork the build has not mirrored, and those files do not exist locally - and the directory is then uploaded by a **second mirror without `--delete`**. Excluding it outright, which is what happened between 2026-09-04 (`f2f8ed1`) and 2026-09-08, silently stopped shipping the `<id>-s.webp` files every built page points at: the build mirrors artwork through sharp as `img/api/<id>-s.webp` while the cron mirrors the same artwork as `<id>.jpg` (no sharp in PHP), so live-rendered cards kept working from the snapshot while **built** pages 404'd. Artwork from before the change kept working, so it read as "only the two newest venues have no logo" until it was 23 of 110 images. `scripts/verify-images.sh` runs at the end of every deploy and fails on a referenced image the server does not have. Note the design assumes the build's webp reaches the server: the cron's `mirror()` prefers `<id>.webp` when it is present, so once uploaded the snapshot switches to it too and the two halves agree. SiteGround's NGINX serves `/data/*.json` with its own six-month `max-age` no matter what `.htaccess` says; the site's fetches use `cache: 'no-cache'` and revalidate, so this only matters for other consumers.
...
- **App invite links (2026-09-05).** The apps share `https://kvizovi.hr/join/team/{code}`, `/join/event/{id}`, `/play/quiz/{id}` and `/get`. `public/.well-known/apple-app-site-association` and `assetlinks.json` (values mirror `DeepLinkSettings.cs` in the PubQuiz API; `tests/unit/well-known.test.ts` pins them) let iOS and Android open those in-app; `deploy.sh` now excludes only `.well-known/acme-challenge/` so the two files mirror with the build. Without the app, `.htaccess` rewrites team/quiz/get links to `server/api/link.php`, which fills the built `dist/link/index.html` (three `{{TOKENS}}`, `tests/unit/link-template.test.ts`) with metadata from the API's `api/links/*` (website key, 5-minute file cache) and points its button at the same path on `api.injeel-it.hr` so 1.x builds still deep-link; event links redirect to `/dogadaji/{id}/?prijava`. `_config.php` holds `ukp_config()` so HTML endpoints can load config without `_common.php`'s JSON headers. **After every deploy that touches these, run `scripts/verify-links.sh`** (Apple CDN and Google checks included) before any app build ships.
